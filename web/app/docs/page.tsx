                ...
                <Card>
                    ...
                    <CardContent className="space-y-6 text-sm">
                        ...
                        <div>
                            ...
                            <CodeBlock>{`GOOGLE_API_KEY=your_gemini_api_key_here\nDATABASE_URL=postgres://reliqa:securepassword@127.0.0.1:5433/reliqa_db\nREDIS_URL=redis://127.0.0.1:6379\nGEMINI_MODEL=gemini-2.5-flash # Optional, defaults to gemini-2.5-flash\n\n# Auth (Google + seed password user via Better Auth)\nGOOGLE_CLIENT_ID=your_google_oauth_client_id\nGOOGLE_CLIENT_SECRET=your_google_oauth_client_secret\nBETTER_AUTH_SECRET=generate_with_openssl_rand_base64_32\nBETTER_AUTH_URL=http://localhost:3000\nAUTH_ALLOWED_EMAILS=you@example.com\nAUTH_SEED_EMAIL=agent@reliqa.local\nAUTH_SEED_PASSWORD=reliqa-agent-pass\nAUTH_SEED_NAME=Reliqa Agent`}</CodeBlock>
                            <p className="text-xs text-muted-foreground mt-2">Google OAuth redirect URI: <code className="bg-muted px-1 rounded">http://localhost:3000/api/auth/callback/google</code>. Allowlisted emails can use Google. Password sign-in is only for the seed user (<code className="bg-muted px-1 rounded">AUTH_SEED_EMAIL</code>) — used when Reliqa tests itself.</p>
                        </div>
